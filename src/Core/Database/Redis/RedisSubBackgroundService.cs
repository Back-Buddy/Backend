    ...
    public class RedisSubBackgroundService(IConnectionMultiplexer connectionMultiplexer, RedisSubBuilder redisSubBuilder, IServiceScopeFactory serviceScopeFactory, ILogger<RedisSubBackgroundService> logger) : BackgroundService
    {
        ...
        internal async Task CallConsumer(IServiceScope scope, Type consumerType, RedisValue message, string channelName)
        {
            try
            {
                ...
                object deserializedMessage = JsonSerializer.Deserialize(message!, messageType, WebSocketConstants.JsonOptions) ?? throw new JsonException("Message could not be parsed!");
